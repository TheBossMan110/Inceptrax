...
import { Compass, Sparkles, Info, Mail, LogOut, ChevronRight, CreditCard } from "lucide-react"
...
export function Navbar() {
  ...
  useEffect(() => {
    // One scroll handler drives both the condensed state and the progress bar.
    // rAF-throttled so a fast wheel can't queue dozens of layout reads.
    let ticking = false
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 pointer-events-none">
      ...
      <div
        aria-hidden
        className="absolute top-0 left-0 right-0 h-[2px] origin-left bg-gradient-to-r from-brand-cyan via-brand to-brand-violet transition-opacity duration-300"
        style={{
      ...
      {/*
        The bar itself never animates height, margin, or width — those force a
        layout pass on every frame, which is what made the old transition feel
        like it was stepping rather than gliding. Instead a fixed-height shell
        holds a glass panel that only scales and fades.
      */}
      <div className="pointer-events-auto relative h-[72px] flex items-center px-4 md:px-8">
        {/* Glass panel — transform + opacity only */}
        <div
          aria-hidden
          className={cn(
            "absolute inset-x-2 md:inset-x-6 top-2 bottom-2 rounded-2xl",
            "glass-strong shadow-[0_8px_40px_-12px_rgba(0,0,0,0.6)]",
            "transition-[opacity,transform] duration-500 ease-[cubic-bezier(0.22,1,0.36,1)]",
            scrolled ? "opacity-100 scale-100" : "opacity-0 scale-[0.98]"
          )}
        ...
        <div className="relative w-full max-w-6xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-8">
            <Link
              href="/"
              className="flex items-center gap-2.5 font-semibold text-lg tracking-tight transition-opacity duration-200 hover:opacity-80"
            >
              <Logo size={30} />
            ...
            <nav className="hidden md:flex items-center gap-0.5 text-sm font-medium">
              {NAV_LINKS.map((link) => (
                <Link
                  key={link.href}
                  href={link.href}
                  className="group relative px-3 py-2 rounded-lg text-muted-foreground hover:text-foreground transition-colors duration-200"
                >
                  {link.label}
                  {/* Underline grows from centre — transform, not width */}
                  <span
                    aria-hidden
                    className="absolute bottom-1 left-3 right-3 h-px origin-center scale-x-0 bg-gradient-to-r from-transparent via-brand to-transparent transition-transform duration-300 ease-out group-hover:scale-x-100"
                  />
                </Link>
              ))}
            </nav>
